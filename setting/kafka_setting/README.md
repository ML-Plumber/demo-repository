...
WSL 환경에서는 `srcs/02-prepare-kafka-network.ps1`을 Windows PowerShell에서 실행한다. 기본값은 Microsoft가 최신 기능과 호환성을 위해 권장하는 mirrored 모드다. NAT가 필요하면 요청한 GNU 형식 또는 일반 PowerShell 형식으로 선택한다.

```powershell
# 기본값: mirrored 모드, Kafka TCP 9092/9093, 원격 범위 LocalSubnet
...
# 같은 의미의 일반 PowerShell 파라미터
.\setting\kafka_setting\srcs\02-prepare-kafka-network.ps1 -NetworkingMode nat

# 실제 peer 주소가 확정되면 기본 LocalSubnet보다 좁게 제한한다.
.\setting\kafka_setting\srcs\02-prepare-kafka-network.ps1 -PeerIps 192.168.10.11,192.168.10.12
```

스크립트는 `%USERPROFILE%\.wslconfig`의 다른 설정을 보존하고 `networkingMode`만 갱신한다. 최초 변경 전 파일은 `.wslconfig.kafka-network.bak`에 보관한다. 설정이 바뀌면 적용을 위해 모든 WSL 배포판을 `wsl --shutdown`으로 종료한 뒤, WSL 내부의 `wslinfo --networking-mode` 결과가 요청값과 일치할 때만 성공으로 처리한다. 적용 결과는 `%ProgramData%\KafkaCluster\network-result.json`에 저장한다.

기본 `PeerIps=LocalSubnet`은 같은 LAN의 가변 노드 수를 허용하기 위한 초기값이다. 실제 노드 IP 또는 CIDR이 확정되면 반드시 `-PeerIps`로 범위를 좁힌다. 스크립트는 native Linux 방화벽, WSL 내부 UFW, 클라우드 보안 그룹과 외부 라우터를 변경하지 않는다.
